#!/bin/bash
source ncrc5.intel23.env
ln -s ncrc5.intel23.env env
...
EXENAME=$clab 
BASEDIR=$(pwd);
MKF_TEMPLATE="$BASEDIR/ncrc5.intel23.mk"
...
echo "Compile FMS"
mkdir -p build/gaea/shared/repro/
(cd build/gaea/shared/repro/; rm -f path_names; \
...
echo "Make NETCDF "
(cd build/gaea/shared/repro/; make clean; make NETCDF=3 REPRO=1 libfms.a -j)

echo "List Model Code paths"
BUILDDIR="build/gaea/$EXENAME/repro/"
mkdir -p $BUILDDIR
...
if [[ "$clab" = "ocean_only" ]]; then
   ...
   (cd $BUILDDIR; rm -f path_names; \
   "$BASEDIR/src/mkmf/bin/list_paths" -v -v -v ./ $BASEDIR/src/MOM6/config_src/{infra/FMS1,memory/dynamic_symmetric,drivers/FMS_cap,external/ODA_hooks,external/drifters,external/stochastic_physics,external/database_comms} $BASEDIR/src/MOM6/pkg/GSW-Fortran/{modules,scripts,toolbox} $BASEDIR/src/MOM6/src/{*,*/*}/ $BASEDIR/src/{atmos_null,SIS2,coupler,land_null,ice_param,icebergs,FMS/coupler,FMS/include}/ $BASEDIR/src/ocean_BGC/{generic_tracers,mocsy/src});
...
(cd $BUILDDIR;make clean; make NETCDF=3 $EXENAME -j)
